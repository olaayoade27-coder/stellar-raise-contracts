 ...
 */
export const DEFAULT_HEADER_CONFIGS: Record<
  AllowedBreakpoint,
...
export class BreakpointValidator {
  ...
  static isValidVisibilityState(visibility: string): boolean {
    if (
      !HEADER_VISIBILITY_STATES.includes(visibility as HeaderVisibilityState)
    ) {
      throw new FrontendHeaderResponsiveError(
...
export class FrontendHeaderResponsive {
  ...
  subscribe(
    id: string,
    callback: (config: HeaderResponsiveConfig) => void,
  ): void {
...
export function useHeaderResponsive(
  initialBreakpoint?: AllowedBreakpoint,
