...
describe("normalizeSubmitButtonLabel", () => {
  ...
  it("strips control characters and normalizes whitespace", () => {
    expect(normalizeSubmitButtonLabel("Pay\u0000Now", "Submit")).toBe(
      "Pay Now",
    );
    expect(normalizeSubmitButtonLabel("Pay\u0008\u001FNow", "Submit")).toBe(
      "Pay Now",
...
describe("resolveSubmitButtonLabel", () => {
  it("returns correct defaults for every state", () => {
    ...
    expect(ALL_STATES.map((s) => resolveSubmitButtonLabel(s))).toEqual(
      expected,
    );
  });
  ...
  it("falls back to defaults for empty or whitespace custom labels", () => {
    ...
    expect(resolveSubmitButtonLabel("submitting", labels)).toBe(
      "Submitting...",
    );
  });
...
describe("isValidSubmitButtonStateTransition", () => {
  ...
  it("blocks transitions not in the allowed map", () => {
    ...
    expect(isValidSubmitButtonStateTransition("success", "submitting")).toBe(
      false,
    );
    expect(isValidSubmitButtonStateTransition("disabled", "submitting")).toBe(
      false,
    );
    expect(isValidSubmitButtonStateTransition("disabled", "success")).toBe(
      false,
    );
  });
});
...
describe("resolveSafeSubmitButtonState", () => {
  it("returns requested state when transition is valid (strict)", () => {
    expect(resolveSafeSubmitButtonState("submitting", "idle", true)).toBe(
      "submitting",
    );
    expect(resolveSafeSubmitButtonState("success", "submitting", true)).toBe(
      "success",
    );
    expect(resolveSafeSubmitButtonState("error", "submitting", true)).toBe(
      "error",
    );
  });
  ...
  it("falls back to previousState for invalid transitions in strict mode", () => {
    ...
    expect(resolveSafeSubmitButtonState("error", "success", true)).toBe(
      "success",
    );
    expect(resolveSafeSubmitButtonState("submitting", "disabled", true)).toBe(
      "disabled",
    );
  });

  it("accepts any state when strict mode is disabled", () => {
    expect(resolveSafeSubmitButtonState("success", "idle", false)).toBe(
      "success",
    );
    expect(resolveSafeSubmitButtonState("error", "success", false)).toBe(
      "error",
    );
  });

  it("accepts requested state when previousState is absent", () => {
    expect(resolveSafeSubmitButtonState("error", undefined, true)).toBe(
      "error",
    );
    expect(resolveSafeSubmitButtonState("success", undefined, true)).toBe(
      "success",
    );
  });
...
describe("isSubmitButtonInteractionBlocked", () => {
  ...
  it("allows interaction for active states with no flags", () => {
    ...
    expect(isSubmitButtonInteractionBlocked("success", false, false)).toBe(
      false,
    );
  });
});
...
describe("isSubmitButtonBusy", () => {
  ...
  it("is false for all non-submitting states with no local flag", () => {
    const nonSubmitting: SubmitButtonState[] = [
      "idle",
      "success",
      "error",
      "disabled",
...
describe("ReactSubmitButton accessibility", () => {
  ...
  it("sets aria-busy='true' while submitting", () => {
    expect(renderBtn({ state: "submitting" }).getAttribute("aria-busy")).toBe(
      "true",
    );
  });

  it("sets aria-busy='false' for non-submitting states", () => {
    const nonBusy: SubmitButtonState[] = [
      "idle",
      "success",
      "error",
      "disabled",
  ...
  it("sets aria-label to the resolved label", () => {
    expect(renderBtn({ state: "idle" }).getAttribute("aria-label")).toBe(
      "Submit",
    );
    expect(renderBtn({ state: "error" }).getAttribute("aria-label")).toBe(
      "Try Again",
    );
  });
});
...
describe("ReactSubmitButton click handling", () => {
  it("fires onClick in idle state", async () => {
    ...
    const { container } = render(
      <ReactSubmitButton state="idle" onClick={onClick} />,
    );
  ...
  it("fires onClick in error state (retry)", async () => {
    const onClick = vi.fn().mockResolvedValue(undefined);
    const { container } = render(
      <ReactSubmitButton state="error" onClick={onClick} />,
    );
    const btn = container.querySelector("button") as HTMLButtonElement;
    await act(async () => {
      fireEvent.click(btn);
